...
class BookingServicer(bookings_pb2_grpc.BookingServiceServicer):
    ...
    def GetBookingByUser(self, request, context):
        ...
        bookings = session.query(models.Booking).filter(
            models.Booking.user_id == request.user_id).all()

        if bookings:
            for booking in bookings:
                booking_array.bookings.extend([bookings_pb2.Booking(id=booking.id, user_id=booking.user_id, listing_id=booking.listing_id, host_id=booking.host_id, start_date=booking.start_date.strftime(
                    '%m/%d/%Y, %H:%M:%S'), end_date=booking.end_date.strftime('%m/%d/%Y, %H:%M:%S'), payment_id=booking.payment_id)])
            context.set_code(grpc.StatusCode.OK)
            return booking_array
        else:
            context.set_code(grpc.StatusCode.NOT_FOUND)
            context.set_details(
                f'Bookings with user id {request.user_id} not found')
            return bookings_pb2.GetBookingArrayResponse()

    def GetBookingByListing(self, request, context):
        booking_array = bookings_pb2.GetBookingArrayResponse()
        bookings = session.query(models.Booking).filter(
            models.Booking.listing_id == request.listing_id).all()

        if bookings:
            for booking in bookings:
                booking_array.extend([bookings_pb2.Booking(id=booking.id, user_id=booking.user_id, listing_id=booking.listing_id, host_id=booking.host_id, start_date=booking.start_date.strftime(
                    '%m/%d/%Y, %H:%M:%S'), end_date=booking.end_date.strftime('%m/%d/%Y, %H:%M:%S'), payment_id=booking.payment_id)])
            context.set_code(grpc.StatusCode.OK)
            return booking_array
        else:
    ...
    def GetAvailableListings(self, request, context):
        ...
        request_start_datetime = datetime.fromtimestamp(
            int(request.start_date.seconds))
        request_end_datetime = datetime.fromtimestamp(
            int(request.end_date.seconds))

        print(
            f'Start date:{request_start_datetime.strftime("%Y-%m-%d %H:%M")}')
        ...
        unavailable_listings_set = {str(tup[0])
                                    for tup in unavailable_listings}
    ...
    def DeleteBookingByUserId(self, request, context):
        result = session.query(models.Booking).filter(
            models.Booking.user_id == request.user_id).all()
        try:
    ...
    def DeleteBookingById(self, request, context):
        print(request)
        result = session.query(models.Booking).filter(
            models.Booking.id == request.booking_id).all()
        print(result)
        try:
            if result:
                for booking in result:
